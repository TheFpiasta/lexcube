...
class AnimationParameters {
    ...
    constructor(
        ...
        sparsity: number,
        updateAnimationDurationLabel: () => void
    ) {
    ...
    private setInitialValues() {
        ...
        this.incrementPerStep = Math.max(
            roundToSparsity((dimensionLength - this.visibleWindow) / targetSteps, this.sparsity),
            this.sparsity
        );
    ...
    private migrateValuesToNewRange() {
        this.visibleWindow = clamp(
            this.visibleWindow,
            this.getRangeForVisibleWindow()[0],
            this.getRangeForVisibleWindow()[1]
        );
        this.incrementPerStep = clamp(
            this.incrementPerStep,
            this.getRangeForIncrementPerStep()[0],
            this.getRangeForIncrementPerStep()[1]
        );
        this.updateStepsAndDuration();
    }
    ...
    private updateStepsAndDuration() {
        this.totalSteps = Math.ceil(
            (this.getRange().length() - this.visibleWindow) / this.incrementPerStep
        );
    ...
    getRangeForVisibleWindow() {
        return [
            this.sparsity,
            Math.max(this.sparsity, roundToSparsity(this.getRange().length() / 2.0, this.sparsity))
        ];
    }

    getRangeForIncrementPerStep() {
        // has to be multiple of sparsity
        return [
            this.sparsity,
            Math.max(
                this.sparsity,
                roundToSparsity(this.getRange().length() / 10.0, this.sparsity)
            )
        ];
    }
    ...
    getAnimationRangeFromStep() {
        ...
        const min = roundUpToSparsity(
            Math.min(range.min + a, range.max - this.visibleWindow - 1),
            this.sparsity
        );
...
class GeospatialContext {
    ...
    setFromDimensions(
        cubeDimensions: CubeDimensions,
        xCorrection: GeospatialContextCorrection,
        yCorrection: GeospatialContextCorrection
    ) {
        ...
        const xBounds = [
            cubeDimensions.x.indices[0] as number,
            cubeDimensions.x.indices[cubeDimensions.x.indices.length - 1] as number
        ];
        const xAscending = xBounds[0] < xBounds[1];
        const xMin = Math.min(...xBounds);
        const xMax = Math.max(...xBounds);
        const yBounds = [
            cubeDimensions.y.indices[0] as number,
            cubeDimensions.y.indices[cubeDimensions.y.indices.length - 1] as number
        ];
...
const CSS_TURN_RED_FILTER =
    'brightness(0) saturate(100%) invert(37%) sepia(72%) saturate(6374%) hue-rotate(344deg) brightness(122%) contrast(117%)';
...
class CubeDimension {
    ...
    constructor(
        ...
        indices: Array<any>,
        attrs: any,
        type: CubeDimensionType | null = null
    ) {
    ...
    private getTimeMetaData() {
        const totalTimeSpanDays = Math.abs(
            ((this.indices[this.indices.length - 1] as Date).getTime() -
                (this.indices[0] as Date).getTime()) /
            (1000 * 60 * 60 * 24)
        );
        const millisecondsPerStep = Math.abs(
            ((this.indices[this.indices.length - 1] as Date).getTime() -
                (this.indices[0] as Date).getTime()) /
            this.indices.length
        );
    ...
    getIndexString(numericIndex: number): string {
        ...
        if (
            this.type == CubeDimensionType.Longitude &&
            this.cubeDimensions.context.interaction.cubeTags.includes(
                CubeTag.LongitudeZeroIndexIsGreenwich
            )
        ) {
    ...
    getDifferenceString(indexDifference: number): { differenceString: string; isExact: boolean } {
        if (this.type == CubeDimensionType.Time) {
            return {
                differenceString: this.getApproximateTimeDifferenceString(indexDifference),
                isExact: false
            };
        }
...
class CubeDimensions {
    ...
    constructor(
        ...
        dimensionSizes: any,
        indices: { x: Array<any>; y: Array<any>; z: Array<any> },
        coords: any
    ) {
        ...
        const coordsGiven =
            ...
            coords[dimensionNames[0]] &&
            coords[dimensionNames[1]] &&
            coords[dimensionNames[2]];
        this.x = new CubeDimension(
            this,
            Dimension.X,
            dimensionNames[2],
            dimensionSizes[dimensionNames[2]],
            indices['x'],
            coordsGiven ? coords[dimensionNames[2]]['attrs'] : undefined
        );
        this.y = new CubeDimension(
            this,
            Dimension.Y,
            dimensionNames[1],
            dimensionSizes[dimensionNames[1]],
            indices['y'],
            coordsGiven ? coords[dimensionNames[1]]['attrs'] : undefined
        );
        this.z = new CubeDimension(
            this,
            Dimension.Z,
            dimensionNames[0],
            dimensionSizes[dimensionNames[0]],
            indices['z'],
            coordsGiven ? coords[dimensionNames[0]]['attrs'] : undefined
        );
    }
    ...
    getGeospatialSubRangeX(first: number, last: number) {
        ...
        return new GeospatialRange(
            this.getGeospatialValueXFromIndex(first),
            this.getGeospatialValueXFromIndex(last),
            this.context.rendering.dimensionOverflow[Dimension.X],
            xRange
        );
    }
    ...
    getGeospatialSubRangeY(first: number, last: number) {
        return new GeospatialRange(
            this.getGeospatialValueYFromIndex(first),
            this.getGeospatialValueYFromIndex(last)
        );
    }
    ...
    setInitialParameterRanges(parameterCoverageTime: ParameterRange) {
        ...
        if (this.z.type == CubeDimensionType.Time && parameterCoverageTime.length() > 0) {
            this.zParameterRange.set(
                int.roundUpToSparsity(parameterCoverageTime.min),
                int.roundDownToSparsity(parameterCoverageTime.max - 1) + 1
            );
        } else {
...
class Parameter {
    constructor(
        name: string,
        sourceData: any,
        attributionData: ParameterAttributionMetadata | undefined,
        colormapData: ParameterColormapMetadata | undefined
    ) {
        this.name = name;
        ...
        this.fixedColormapMinimumValue =
            colormapData?.colormapMinimumValue !== undefined
                ? colormapData.colormapMinimumValue
                : undefined;
        this.fixedColormapMaximumValue =
            colormapData?.colormapMaximumValue !== undefined
                ? colormapData.colormapMaximumValue
                : undefined;
        this.fixedColormap = colormapData?.colormap || undefined;
        this.fixedColormapFlipped = colormapData?.colormapFlipped || false;
        this.sourceData = sourceData;
        this.parameterCoverageTime = new ParameterRange(
            sourceData['first_valid_time_slice'],
            sourceData['last_valid_time_slice'] + 1
        );
        this.patchMetadata();
    }
    ...
    private patchMetadata() {
        if (this.name == 'precipitation_era5') {
            ...
            ['k', 'kelvin'].includes(this.units.toLowerCase()) &&
            this.globalMaximumValue > 273.15
        ) {
            this.units = '°C';
...
class GeospatialRange {
    constructor(
        first: number,
        last: number,
        overflowAllowed: boolean = false,
        overflowRange: number = 0
    ) {
        this.first = first;
        this.last = last;
...
class ParameterRange {
    ...
    public toString(maxOffset: number = 0, roundedToSparsity: boolean = false) {
        const min = roundedToSparsity
            ? roundUpToSparsity(this.min, ParameterRange.sparsity)
            : this.min;
        const max = roundedToSparsity
            ? roundDownToSparsity(this.max, ParameterRange.sparsity) + maxOffset
            : this.max + maxOffset;
        return `${min}-${max}`;
    }
...
class CubeSelection {
    ...
    constructor(context: CubeClientContext) {
        ...
        if (!context.widgetMode) {
            if (context.interaction.XYdataAspectRatio > 1.0) {
                this.xSelectionRange = new ParameterRange(
                    0,
                    context.interaction.roundDownToSparsity(
                        dims.xParameterRange.max / context.interaction.XYdataAspectRatio
                    ) + 1,
                    true
                );
            } else {
                this.ySelectionRange = new ParameterRange(
                    0,
                    context.interaction.roundDownToSparsity(
                        dims.yParameterRange.max * context.interaction.XYdataAspectRatio
                    ) + 1,
                    true
                );
            }
        ...
        if (context.interaction.cubeTags.includes(CubeTag.ESDC)) {
            ...
            this.xSelectionRange.set(
                offset,
                this.context.interaction.roundDownToSparsity(offset + l - 1) + 1
            );
        }
        if (
            context.interaction.cubeTags.includes(CubeTag.CamsEac4Reanalysis) ||
            context.interaction.cubeTags.includes(CubeTag.Era5SpecificHumidity)
        ) {
        ...
        if (this.context.interaction.initialLoad) {
            this.parseInitialRange(
                is.xRange,
                dims.xParameterRange,
                this.xSelectionRange,
                Dimension.X
            );
            this.parseInitialRange(
                is.yRange,
                dims.yParameterRange,
                this.ySelectionRange,
                Dimension.Y
            );
            this.parseInitialRange(
                is.zRange,
                dims.zParameterRange,
                this.zSelectionRange,
                Dimension.Z
            );
        }
    ...
    private parseInitialRange(
        parsedRange: number[] | undefined,
        ...
        const s = new ParameterRange(
            this.context.interaction.roundUpToSparsity(parsedRange[0]),
            this.context.interaction.roundDownToSparsity(parsedRange[1]) + 1
        );
        if (
            s.length() > 0 &&
            s.subRangeOf(parameterRange, this.context.rendering.dimensionOverflow[dimension])
        ) {
    ...
    parseSelectionBoundariesFromWidget(
        xMin: number,
        xMax: number,
        yMin: number,
        yMax: number,
    ...
    parseSelectionBoundary(
        parsedLowerBoundary: number | undefined,
        parsedUpperBoundary: number | undefined,
        dimension: Dimension
    ) {
        if (
            typeof parsedLowerBoundary != 'number' ||
            isNaN(parsedLowerBoundary) ||
            parsedLowerBoundary < 0 ||
            typeof parsedUpperBoundary != 'number' ||
            isNaN(parsedUpperBoundary) ||
            parsedUpperBoundary < 0
        ) {
            return false;
        }
        ...
        const parameterRange =
            this.context.interaction.cubeDimensions.getParameterRangeByDimension(dimension);
        if (
            attemptedRange.length() >= 1 &&
            attemptedRange.subRangeOf(
                parameterRange,
                this.context.rendering.dimensionOverflow[dimension]
            )
        ) {
            ...
            throw new Error(
                `Invalid selection boundary ${attemptedRange} for dimension ${Dimension[dimension]} (Valid parameter range is ${parameterRange})`
            );
        }
    }

    private roundVectorToSparsity(
        vector: Vector2,
        minX: number,
        maxX: number,
        minY: number,
    ...
    private roundSizeToSparsity(size: Vector2, face: CubeFace) {
        ...
        const v = this.roundVectorToSparsity(size.clone().subScalar(1), 0, maxX, 0, maxY).addScalar(
            1
        );
        return v;
    }
    ...
    private roundOffsetToSparsity(offset: Vector2, face: CubeFace) {
        ...
        const max = this.context.interaction.getMaximumDisplayOffset(
            face,
            this.displaySizes[Math.floor(face / 2)]
        );
        if (
            this.context.interaction.cubeTags.includes(CubeTag.OverflowX) &&
            (face == CubeFace.Front ||
                face == CubeFace.Back ||
                face == CubeFace.Top ||
    ...
    fixAllVectorsToSparsity() {
        for (let i = 0; i < 3; i++) {
            ...
            if (!to.equals(this.displayOffsets[i])) {
                console.warn(
                    'Fixing offset vector',
                    i,
                    'currently',
                    this.displayOffsets[i],
                    'now',
                    to
                );
    ...
    private updateAfterVectorChange(face: CubeFace, finalChange: boolean) {
        ...
        if (this.context.orchestrationMinionMode || this.context.orchestrationMasterMode) {
            this.context.networking.pushOrchestratorSelectionUpdate(
                this.displayOffsets,
                this.displaySizes,
                finalChange
            );
    ...
    setRange(dimension: Dimension, min: number, max: number) {
        ...
        range.set(
            this.context.interaction.roundToSparsity(min),
            this.context.interaction.roundToSparsity(max) + 1
        );
        this.updateAllVectors();
        if (this.context.orchestrationMinionMode || this.context.orchestrationMasterMode) {
            this.context.networking.pushOrchestratorSelectionUpdate(
    ...
    private updateRanges(face: CubeFace, finalChange: boolean) {
        this.xSelectionRangeForFace(face).set(
            this.displayOffsets[Math.floor(face / 2)].x,
            this.displayOffsets[Math.floor(face / 2)].x + this.displaySizes[Math.floor(face / 2)].x,
            finalChange,
            this.displaySizes[Math.floor(face / 2)].x
        );
        this.ySelectionRangeForFace(face).set(
            this.displayOffsets[Math.floor(face / 2)].y,
            this.displayOffsets[Math.floor(face / 2)].y + this.displaySizes[Math.floor(face / 2)].y,
            finalChange,
            this.displaySizes[Math.floor(face / 2)].y
        );
    }
    ...
    getIndexValueForFace(face: CubeFace): number {
        if (face == CubeFace.Front) {
            ...
            return positiveModulo(
                this.xSelectionRange.min,
                this.context.interaction.cubeDimensions.x.steps
            );
        } else {
            return positiveModulo(
                this.xSelectionRange.max - 1,
                this.context.interaction.cubeDimensions.x.steps
            );
        }
    }
...
class CubeInteraction {
    ...
    private cameraPresets = [
        {
            name: 'Diagonal Close-up South America',
            position: new Vector3(0.7334182744080036, -0.1937720441909164, 0.23593831568307924),
            rotation: new Euler(0.6875841168575725, 1.17633108419973, -0.6487348071441518, 'XYZ')
        },
        {
            name: 'Full Earth - Front',
            position: new Vector3(2.080720175325273, 6.61750251743031e-17, 6.61750251743031e-17),
            rotation: new Euler(-6.162975822039155e-33, 1.5707963267948966, 0)
        },
        {
            name: 'Look at right side',
            position: new Vector3(0.005694911428844007, 0.030379652376199662, -1.1831101491937133),
            rotation: new Euler(-3.115920506235054, 0.004811885820327965, 3.1414690954800735)
        },
        {
            name: 'Tilted, front/left/top',
            position: new Vector3(1.795268175811992, 0.8548055731845162, 1.0628454932761202),
            rotation: new Euler(-0.7079403135831094, 0.9088139060848945, 0.5938561528919046)
        },
        {
            name: 'Very far away',
            position: new Vector3(13.336517975968864, 2.399780986066818, 3.0129436502811733),
            rotation: new Euler(-0.6725973350194719, 1.2896276568959506, 0.653167025497404)
        },
        {
            name: 'Multi-cube',
            position: new Vector3(5.72725123221618, 0, 0),
            rotation: new Euler(0.9273842476318528, 0.5 * Math.PI, -0.9269676945976868)
        },
        {
            name: 'Single Face (Front)',
            position: new Vector3(1, 0, 0),
            rotation: new Euler(0, Math.PI / 2, 0)
        },
        {
            name: 'Single Face (Back)',
            position: new Vector3(-1, 0, 0),
            rotation: new Euler(Math.PI, -Math.PI / 2, Math.PI)
        },
        {
            name: 'Single Face (Top)',
            position: new Vector3(1e-5, 1, 0),
            rotation: new Euler(-Math.PI / 2, 1e-8, Math.PI / 2)
        },
        {
            name: 'Single Face (Bottom)',
            position: new Vector3(1e-5, -1, 0),
            rotation: new Euler(Math.PI / 2, 0, -Math.PI / 2)
        },
        {
            name: 'Single Face (Left)',
            position: new Vector3(0, 0, 1),
            rotation: new Euler(0, 0, 0)
        },
        {
            name: 'Single Face (Right)',
            position: new Vector3(0, 0, -1),
    ...
    private getHtmlElementByClassName(className: string): HTMLElement {
        ...
        if (elements.length != 1) {
            console.warn(
                'Tried to access HTML element of class name',
                className,
                'but got',
                elements.length,
                'results.'
    ...
    private setupHtmlReferences() {
        this.htmlQualitySelect = this.getHtmlElementByClassName(
            'quality-select'
        )! as HTMLSelectElement;
        this.htmlCubeSelect = this.getHtmlElementByClassName('cube-select')! as HTMLSelectElement;
        this.htmlParameterSelect = this.getHtmlElementByClassName(
            'parameter-select'
        )! as HTMLSelectElement;
        this.htmlColormapFlippedCheckbox = this.getHtmlElementByClassName(
            'colormap-flipped-checkbox'
        )! as HTMLInputElement;
        this.htmlColormapPercentileCheckbox = this.getHtmlElementByClassName(
            'colormap-percentile-checkbox'
        )! as HTMLInputElement;
        this.htmlColormapButtonList = this.getHtmlElementByClassName(
            'colormap-list'
        )! as HTMLDivElement;
        ...
        this.htmlAnimationIncrementSliderDiv = this.getHtmlElementByClassName(
            'animation-increment-slider'
        )!;
        this.htmlAnimationWindowSliderDiv =
            this.getHtmlElementByClassName('animation-window-slider')!;
        this.htmlAnimationSpeedSliderDiv = this.getHtmlElementByClassName('animation-speed-slider')!;

        this.htmlAnimationTotalDurationDiv = this.getHtmlElementByClassName(
            'animation-total-duration'
        )!;

        this.htmlAnimationDropdown = this.getHtmlElementByClassName('animation-dropdown-content')!;

        this.htmlAnimationRecordingCheckbox = this.getHtmlElementByClassName(
            'animation-recording-checkbox'
        )! as HTMLInputElement;
        this.htmlAnimationRecordingCheckboxLabel = this.getHtmlElementByClassName(
            'animation-recording-checkbox-label'
        )!;
        this.htmlAnimationSelectedRangeOnlyCheckbox = this.getHtmlElementByClassName(
            'animation-selected-range-only-checkbox'
        )! as HTMLInputElement;
        this.htmlAnimationSelectedRangeOnlyCheckboxLabelDiv = this.getHtmlElementByClassName(
            'animation-selected-range-only-checkbox-label-div'
        )!;
        this.htmlAnimationRecordingInProgressPanel = this.getHtmlElementByClassName(
            'animation-recording-in-progress-panel'
        )!;
        this.htmlAnimationRecordingOptions = this.getHtmlElementByClassName(
            'animation-recording-options'
        )!;
        this.htmlAnimationRecordingFormatSelect = this.getHtmlElementByClassName(
            'animation-recording-format'
        )! as HTMLSelectElement;
        this.htmlAnimationRecordingRestartButton = this.getHtmlElementByClassName(
            'animation-recording-restart-button'
        )! as HTMLButtonElement;
        this.htmlAnimationRecordingStopButton = this.getHtmlElementByClassName(
            'animation-recording-stop-button'
        )! as HTMLButtonElement;
        this.htmlAnimationDimensionSelect = this.getHtmlElementByClassName(
            'animation-dimension-select'
        )! as HTMLSelectElement;

        this.htmlColormapRangeForm = this.getHtmlElementByClassName(
            'colormap-range-form'
        )! as HTMLFormElement;
        this.htmlColormapOptions = this.getHtmlElementByClassName(
            'colormap-options'
        )! as HTMLElement;

        this.htmlColormapMinInputDiv = this.getHtmlElementByClassName(
            'colormap-min-input'
        ) as HTMLInputElement;
        this.htmlColormapMaxInputDiv = this.getHtmlElementByClassName(
            'colormap-max-input'
        ) as HTMLInputElement;
        ...
        this.htmlDownloadPrintTemplateButton = this.getHtmlElementByClassName(
            'download-template-button'
        )!;
        this.htmlPrintTemplateResultWrapper = this.getHtmlElementByClassName(
            'print-template-result-wrapper'
        )!;

        this.htmlPrintTemplateResult = this.getHtmlElementByClassName('print-template-result')!;
        this.htmlPrintTemplateDownloadButtonPng = this.getHtmlElementByClassName(
            'download-print-template-result-png'
        )! as HTMLAreaElement;
        this.htmlPrintTemplateDownloadButtonSvg = this.getHtmlElementByClassName(
            'download-print-template-result-svg'
        )! as HTMLAreaElement;
        this.htmlPrintTemplateDownloadEditNoteButton = this.getHtmlElementByClassName(
            'download-print-template-result-edit-note'
        )! as HTMLAreaElement;

        this.htmlPrintTemplateLoadingSection = this.getHtmlElementByClassName(
            'print-template-loading-section'
        )!;
        this.htmlPrintTemplateLoaderVideo = this.getHtmlElementByClassName(
            'print-template-loader-video'
        )! as HTMLVideoElement;
        this.htmlPrintTemplateResultSection = this.getHtmlElementByClassName(
            'print-template-result-section'
        )!;
        ...
        this.htmlAxisLabelXDimensionName = this.getHtmlElementByClassName(
            'axis-label-x-dimension-name'
        )!;
        this.htmlAxisLabelXDimensionNameParent = this.getHtmlElementByClassName(
            'axis-label-parent-x-dimension-name'
        )!;
        ...
        this.htmlAxisLabelYDimensionName = this.getHtmlElementByClassName(
            'axis-label-y-dimension-name'
        )!;
        this.htmlAxisLabelYDimensionNameParent = this.getHtmlElementByClassName(
            'axis-label-parent-y-dimension-name'
        )!;
        ...
        this.htmlAxisLabelZDimensionName = this.getHtmlElementByClassName(
            'axis-label-z-dimension-name'
        )!;
        this.htmlAxisLabelZDimensionNameParent = this.getHtmlElementByClassName(
            'axis-label-parent-z-dimension-name'
        )!;
    ...
    private registerEvents() {
        ...
        this.orbitControls = new OrbitControls(
            this.context,
            this.context.rendering.mainCamera,
            domElement
        );
        ...
        this.orbitControls.addEventListener(
            'end',
            this.context.rendering.updateVisibilityAndLods.bind(this.context.rendering)
        );
        ...
        domElement.addEventListener(
            ...
            (ev: TouchEvent) => {
                ...
                this.hideMenusForTouchDevices();
                this.currentTouchEventOnCube = !Array.from(ev.touches).some(
                    (value: Touch, index: number, array: Touch[]) => {
                        return isOverBackground(this.getLocalEventPosition(value));
                    }
                )
    ...
    updateLabelPositions() {
        ...
        const angleToTranslate = (v: number) =>
            `translate(-${clamp(200 * (0.75 - Math.abs(1 - v)), 0, 100)}%, -${clamp(200 * (0.75 - Math.abs(1 - ((v + 1.5) % 2))), 0, 100)}%)`;
    ...
    private triggerDeferredVisibilityAndLodUpdate() {
        ...
        this.deferredVisibilityAndLodUpdateTimeoutHandler = window.setTimeout(
            () => this.context.rendering.updateVisibilityAndLods(),
            this.deferredVisibilityAndLodUpdateMilliseconds
        );
    }
    ...
    private finishInteraction() {
        if (this.interactionFinishDisplayOffset) {
            ...
            this.cubeSelection.setVectors(
                this.interactionFinishFace!,
                this.interactionFinishDisplaySize || new Vector2(),
                this.interactionFinishDisplayOffset
            );
        }
        ...
        this.previousZoomFactor[Math.floor(this.currentZoomFace / 2)] =
            this.currentZoomFactor[Math.floor(this.currentZoomFace / 2)];
    ...
    onPanStart(initialPosition: Vector2) {
        const ray = this.context.rendering.raycastWindowPosition(
            initialPosition.x,
            initialPosition.y
        );
    ...
    private getXOverflowEnabledForFace(face: CubeFace) {
        // in case of X, local X and data X are the same
        return (
            face != CubeFace.Left &&
            face != CubeFace.Right &&
            this.context.rendering.dimensionOverflow[Dimension.X]
    ...
    onPanMove(currentPosition: Vector2) {
        ...
        const uvDifference = new Vector2(
            ray[0].uv!.x - this.panStartUv.x,
            ray[0].uv!.y - this.panStartUv.y
        );
        ...
        if (xOverflowEnabled) {
            newDisplayOffset.y = clamp(
                newDisplayOffset.y,
                minimumDisplayOffset.y,
                maximumDisplayOffset.y
            );
        ...
        this.triggerMaxRangeIndicatorsFromOffsetIfScrolledOutSignificantly(
            face,
            unclampedNewDisplayOffset,
            displaySize
        );
    }
    ...
    onZoom(eventPositions: (MouseEvent | Touch)[], zoomDelta: number, immediate: boolean = false) {
        ...
        for (let i = 0; i < eventPositions.length; i++) {
            ...
            ray = this.context.rendering.raycastWindowPosition(
                localEventPosition.x,
                localEventPosition.y
            );
            if (ray && ray[0]) {
                break;
            }
        ...
        if (eventPositions.length == 2) {
            const middle = this.getLocalEventPosition(eventPositions[0])
                .add(this.getLocalEventPosition(eventPositions[1]))
                .multiplyScalar(0.5);
        ...
        if (Math.abs(zoomDelta) > 0.001) {
            this.changeZoomOnFace(
                Math.sign(zoomDelta) * clamp(Math.abs(zoomDelta), 0.001, 20.0) * 3,
                immediate ? r[0].face!.materialIndex : this.currentZoomFace,
                uv,
                immediate
            );
            return true;
    ...
    private updateColormapOverrideRangesFromUi(updateColormap: boolean = true) {
        ...
        if (
            ...
            !isNaN(parseFloat(this.htmlColormapMinInputDiv.value))
        ) {
        ...
        if (
            ...
            !isNaN(parseFloat(this.htmlColormapMaxInputDiv.value))
        ) {
        ...
        if (this.context.widgetMode) {
            this.updateWidgetModelColormapRange(
                td.colormapMinValueOverride,
                td.colormapMaxValueOverride
            );
        }
        if (updateColormap) {
    ...
    private updateHoverInfo(mousePosition: Vector2 | undefined = undefined) {
        ...
        hoverPosition.x = positiveModulo(
            hoverPosition.x,
            this.cubeDimensions.totalWidthForFace(face)
        );
        ...
        this.hoverData.x =
            face <= 3 ? hoverPosition.x : this.cubeSelection.getIndexValueForFace(face);
        this.hoverData.y =
            face > 3
                ? hoverPosition.x
                : face <= 1
                    ? hoverPosition.y
                    : this.cubeSelection.getIndexValueForFace(face);
        this.hoverData.z =
            face > 1 ? hoverPosition.y : this.cubeSelection.getIndexValueForFace(face);
        this.hoverData.maximumCompressionError = this.context.tileData.maxCompressionErrors.get(
            new Tile(
                face,
                face <= 1 ? this.hoverData.z : face <= 3 ? this.hoverData.y : this.hoverData.x,
                lod,
                tileX,
                tileY,
                this.selectedCube.id,
    ...
    private updateHoverInfoUi() {
        ...
        lines.push(
            `${this.cubeDimensions.z.getName()}: ${this.cubeDimensions.z.getIndexString(this.hoverData.z)}${this.context.debugMode ? ` (Z / ${this.hoverData.z})` : ''}`
        );
        lines.push(
            `${this.cubeDimensions.y.getName()}: ${this.cubeDimensions.y.getIndexString(this.hoverData.y)}${this.context.debugMode ? ` (Y / ${this.hoverData.y})` : ''}`
        );
        lines.push(
            `${this.cubeDimensions.x.getName()}: ${this.cubeDimensions.x.getIndexString(this.hoverData.x)}${this.context.debugMode ? ` (X / ${this.hoverData.x})` : ''}`
        );
        if (this.context.debugMode) {
            lines.push(
                `Max. error introduced by compression in this tile: ${this.hoverData.maximumCompressionError}`
            );
            lines.push(`Face: ${CubeFace[this.hoverData.face]} (${this.hoverData.face})`);
            lines.push(
                `Tile x: ${this.hoverData.tileX} y: ${this.hoverData.tileY}, Pixel x: ${this.hoverData.pixelX} y: ${this.hoverData.pixelY}`
            );
            lines.push(
                `Display Quality: ${(100 * Math.pow(0.5, this.context.rendering.lods[this.hoverData.face])).toFixed(2)}% (LoD ${this.context.rendering.lods[this.hoverData.face]})`
            );
        }
    ...
    updateColormapRangeUiFromValues() {
        this.htmlColormapMinInputDiv.value =
            this.context.tileData.colormapMinValueOverride !== null
                ? this.toFixed(this.context.tileData.colormapMinValueOverride)
                : '';
        this.htmlColormapMaxInputDiv.value =
            this.context.tileData.colormapMaxValueOverride !== null
                ? this.toFixed(this.context.tileData.colormapMaxValueOverride)
                : '';
    }
    ...
    updateColormapRangePlaceholders() {
        ...
        if (this.context.expertMode) {
            this.htmlColormapMinInputDiv.placeholder = td.colormapUseStandardDeviation
                ? `${this.toFixed(td.statisticalColormapLowerBound)} (${td.statisticalColormapLowerBound == td.observedMinValue ? 'same' : this.toFixed(td.observedMinValue)})`
                : this.toFixed(td.observedMinValue);
            this.htmlColormapMaxInputDiv.placeholder = td.colormapUseStandardDeviation
                ? `${this.toFixed(td.statisticalColormapUpperBound)} (${td.statisticalColormapUpperBound == td.observedMaxValue ? 'same' : this.toFixed(td.observedMaxValue)})`
                : this.toFixed(td.observedMaxValue);
        } else {
            this.htmlColormapMinInputDiv.placeholder =
                td.colormapUseStandardDeviation && !td.ignoreStatisticalColormapBounds
                    ? `${this.toFixed(td.statisticalColormapLowerBound)}`
                    : this.toFixed(td.observedMinValue);
            this.htmlColormapMaxInputDiv.placeholder =
                td.colormapUseStandardDeviation && !td.ignoreStatisticalColormapBounds
                    ? `${this.toFixed(td.statisticalColormapUpperBound)}`
                    : this.toFixed(td.observedMaxValue);
        }
    }
    ...
    updateStatusMessage(
        tileDownloadsTriggered?: number,
        tileDownloadsFinished?: number,
        tileDownloadsFailed?: number,
        tileDecodesFailed?: number
    ) {
        if (
            tileDownloadsTriggered !== undefined &&
            tileDownloadsFinished !== undefined &&
            tileDownloadsFailed !== undefined &&
            tileDecodesFailed !== undefined
        ) {
            this.lastStatusMessageProgress = [
                tileDownloadsTriggered,
                tileDownloadsFinished,
                tileDownloadsFailed,
                tileDecodesFailed
            ];
        }
                ...
                if (this.requestProgressTimingEnabled) {
                    ...
                    const estimatedFinishTime =
                        (overestimationFactor *
                            (this.requestProgressLastUpdate - this.requestProgressStart)) /
                        p +
                        this.requestProgressStart +
                        overestimationFlat;
                    const estimatedRemainingTime = estimatedFinishTime - now;
                    const estimatedRemainingTimeSeconds = Math.max(
                        Math.round(estimatedRemainingTime / 1000),
                        1
                    );
                    if (p < 1 && p > 0) {
                        if (estimatedRemainingTimeSeconds < 3) {
                            ...
                            lines.push(
                                `<i>ca. ${estimatedRemainingTimeSeconds} second${estimatedRemainingTimeSeconds != 1 ? 's' : ''} remaining</i>`
                            );
                        }
                        window.setTimeout(() => {
                            this.updateStatusMessage();
        ...
        if (downloadsFailed > 0) {
            lines.push(
                this.context.expertMode
                    ? `${tileDownloadsFailed} tile downloads failed`
                    : 'Some downloads failed - try refreshing?'
            );
        }
        if (decodeFailed > 0) {
            lines.push(
                this.context.expertMode
                    ? `${tileDecodesFailed} tile decodes failed`
        ...
        if (this.connectionLostMessageVisible) {
            lines.push(
                'It seems the server connection was lost.<br>Please reconnect to the internet.'
            );
        }
            ...
            for (const line of lines) {
                const color =
                    line.indexOf('version') > -1
                        ? '#48eeff'
                        : line.indexOf('failed') > -1 || line.indexOf('went wrong') > -1
                            ? '#ff4444'
                            : 'white';
                html += `<div style='color: ${color}'>${line}</div>`;
            }
    ...
    private setupHtmlInterface() {
        ...
        this.htmlQualitySelect.onchange = () => {
            this.context.rendering.displayQuality = parseFloat(
                this.htmlQualitySelect.selectedOptions[0].value
            );
            this.context.rendering.updateVisibilityAndLods();
        };
        ...
        this.htmlColormapPercentileCheckbox.onchange = () => {
            this.context.tileData.colormapUseStandardDeviation =
                this.htmlColormapPercentileCheckbox.checked;
            this.context.tileData.colormapHasChanged(true, false);
        };
        ...
        this.htmlAnimationRecordingCheckbox.onchange = () => {
            ...
            this.htmlAnimateStartButton.style.filter = this.recordAnimation
                ? CSS_TURN_RED_FILTER
                : '';
            this.htmlAnimationRecordingOptions.style.display = this.recordAnimation
                ? 'contents'
                : 'none';
        };
        ...
        this.htmlAnimationRecordingFormatSelect.onchange = () => {
            this.context.rendering.setAnimationRecordingFormat(
                this.htmlAnimationRecordingFormatSelect.value
            );
        };

        this.htmlAnimationDimensionSelect.onchange = () => {
            this.updateAnimationDimension(
                Dimension[
                    this.htmlAnimationDimensionSelect.value.toUpperCase() as keyof typeof Dimension
                    ]
            );
        };
        ...
        this.htmlColormapScale.onclick = () => {
            this.htmlColormapOptions.style.display =
                this.htmlColormapOptions.style.display == 'flex' ? 'none' : 'flex';
                ...
                for (let categoryHeaderOrContainer of this.htmlColormapButtonList.children) {
                    ...
                    for (let f of categoryHeaderOrContainer.children) {
                        if (f.classList.contains('selected')) {
                            ...
                            f.scrollIntoView({
                                block: this.context.widgetMode ? 'nearest' : 'center'
                            });
                        }
                    }
                }
        ...
        this.datasetInfoDialogWrapperDiv.onclick = () =>
            (this.datasetInfoDialogWrapperDiv.style.display = 'none');
        this.getHtmlElementByClassName('dataset-info-window')!.onclick = (ev) => {
            ev.stopPropagation();
        };
    }
     ...
     */
    private mergeSliderTooltips(
        slider: HTMLElement & { noUiSlider: any },
        threshold: number,
        separator: string
    ) {
        ...
        slider.noUiSlider.on(
            'update',
            function(values: any, handle: any, unencoded: any, tap: any, positions: any) {
                var pools: number[][] = [[]];
                ...
                pools.forEach(function(pool, poolIndex) {
                    ...
                    for (var j = 0; j < handlesInPool; j++) {
                        ...
                        if (j === handlesInPool - 1) {
                            ...
                            if (formatter) {
                                tooltips[handleNumber].innerHTML = poolValues[poolIndex]
                                    .map((str: string) => formatter.to(parseInt(str)))
                                    .join(separator);
                            } else {
                            ...
                            if (handlesInPool > 1) {
                                tooltips[handleNumber].style['right'] = isRight
                                    ? offset + '%'
                                    : 'auto';
                                tooltips[handleNumber].style['left'] = !isRight
                                    ? 30 - offset + '%'
                                    : 'auto';
                            } else {
    ...
    private gpsPositionReceived(position: { coords: { latitude: number; longitude: number } }) {
        ...
        const relativeLatitude = this.cubeDimensions
            .getGeospatialTotalRangeY()
            .relativeWithin(-crd.latitude);
        const relativeLongitude = this.cubeDimensions
            .getGeospatialTotalRangeX()
            .relativeWithin(crd.longitude);
        const threshold = 0.2;
        if (
            relativeLatitude < -threshold ||
            relativeLongitude < -threshold ||
            relativeLatitude > 1 + threshold ||
            relativeLongitude > 1 + threshold
        ) {
            window.alert(
                'Your current location is not within the bounds of the cube. GPS will be deactivated.'
            );
            this.stopGps();
            return;
        }
    ...
    private startGps() {
        if (!this.geospatialContextProvided) {
            return window.alert(
                'Currently selected cube does not have geospatial context, cannot enable GPS position tracking.'
            );
        }
        this.gpsTrackingEnabled = true;
        this.htmlGpsButton.style.filter = 'drop-shadow(0px 0px 6px #fff)';
        this.gpsTrackingId = navigator.geolocation.watchPosition(
            this.gpsPositionReceived.bind(this),
            this.gpsPositionError.bind(this),
            {
    ...
    private prepareSlider(
        div: HTMLElement,
        parameterUpdate: (arr: string[]) => void,
        viewUpdate: () => void,
    ...
    private updateAnimationSliders() {
        ...
        const windowRange = this.animationParameters.getExponentialRangeFromLinearRange(
            this.animationParameters.getRangeForVisibleWindow()
        );
        ...
        const incrementRange = this.animationParameters.getExponentialRangeFromLinearRange(
            this.animationParameters.getRangeForIncrementPerStep()
        );
    ...
    private prepareAnimationSliders() {
        ...
        this.animationIncrementSlider.updateOptions(
            { tooltips: windowAndIncrementFormatter },
            false
        );
    ...
    private updateSelectionUiRangeBounds() {
        ...
        this.zSelectionSlider.updateOptions(
            {
                range: { min: zRange.min, max: zRange.max - 1 },
                step: this.selectedCubeMetadata.sparsity,
                margin: this.selectedCubeMetadata.sparsity
            },
            false
        );
        this.ySelectionSlider.updateOptions(
            {
                range: { min: yRange.min, max: yRange.max - 1 },
                step: this.selectedCubeMetadata.sparsity,
                margin: this.selectedCubeMetadata.sparsity
            },
            false
        );
        this.xSelectionSlider.updateOptions(
            {
                range: { min: xRange.min, max: xRange.max - 1 },
                step: this.selectedCubeMetadata.sparsity,
                margin: this.selectedCubeMetadata.sparsity
            },
            false
    ...
    updateSlidersAndLabelsAfterChange(
        updateSliders: boolean = true,
        updateLabels: boolean = true,
        dimensionsOnly: Dimension[] = []
    ) {
        if (updateSliders) {
    ...
    updateSliderValuesAfterChange(dimensionsOnly: Dimension[] = []) {
        ...
        const sliderOffset = this.cubeTags.includes(CubeTag.LongitudeZeroIndexIsGreenwich)
            ? this.roundUpToSparsity(lonRange.length() / 2)
            : 0;
        ...
        const overflowXIndex = Math.floor(
            (xSelectionRange.min + sliderOffset) / this.cubeDimensions.x.steps + overflowBias
        );
        if (overflowXIndex != this.lastOverflowXSliderIndex) {
            const newMinimum = this.roundDownToSparsity(
                overflowXIndex * this.cubeDimensions.x.steps + lonRange.min + sliderOffset
            );
            const newMaximum = this.roundDownToSparsity(
                overflowXIndex * this.cubeDimensions.x.steps + lonRange.max + sliderOffset - 1
            );
            this.xSelectionSlider.updateOptions(
                { range: { min: newMinimum, max: newMaximum } },
                false
            );
        ...
        if (dimensionsOnly.length == 0 || dimensionsOnly.includes(Dimension.X)) {
            this.xSelectionSlider.set(
                [
                    xSelectionRange.min + sliderOffset * 2,
                    xSelectionRange.max + sliderOffset * 2 - 1
                ],
                false
            ); // sliderOffset*2 is hacky, but works for the two data sets with zeroIndexIsGreenwich
        }
    }
    ...
    private getAttributionParameterMetadata(parameter: string) {
        let id =
            this.selectedCube.shortName.indexOf('Hainich') > -1 ? 'Hainich' : this.selectedCube.id;
    ...
    private updateDatasetInfoAndShow(
        updateParameterInfo: boolean = true,
        showPopup: boolean = false
    ) {
        const makeLink = (link: string, linkText?: string) =>
            `<a target="_blank" rel="noopener" href='${link}'>${linkText || link}</a>`;
        let dialogLines: string[] = [];
        let cornerLines: string[] = [];
        const dialogParameterHeading = (value: string) =>
            dialogLines.push(`<div><b>${value}</b></div>`);
        const dialogParameterValue = (value: string) =>
            dialogLines.push(`<div style="margin-left:5%;margin-bottom:8px">${value}</b></div>`);
        const cornerLineBold = (value: string) => {
            if (value && value.length > 0) cornerLines.push(`<div><b>${value}</b></div>`);
        };
        const cornerLineUnderline = (value: string) => {
            if (value && value.length > 0)
                cornerLines.push(`<div style="text-decoration:underline;"><a>${value}</a></div>`);
        };
        const cornerLine = (value: string) => {
            if (value && value.length > 0) cornerLines.push(`<div>${value}</div>`);
        };
        const cornerLineSmall = (value: string) => {
            if (value && value.length > 0)
                cornerLines.push(`<div style='font-size: 70%;'>${value}</div>`);
        };
        ...
        if (p && p.long_name) {
            ...
            if (p['project_name'])
                parameterLines.set(
                    'Source Project:',
                    `${p['project_name']} (<a target="_blank" rel="noopener" href='${p['dataset_link']}'>${p['dataset_link']}</a>)`
                );
            ...
            if (this.selectedParameter.project)
                cornerLine(`Data Source: ${this.selectedParameter.project}`);

            cornerLineUnderline(`Data attribution and license`);
            if (attrs['acknowledgment'])
                parameterLines.set(
                    'Acknowledgment:',
                    `${attrs['acknowledgment']} (<a target="_blank" rel="noopener" href='${attrs['source']}'>${attrs['source']}</a>)`
                );
            parameterLines.set('Description:', attrs['description']);
            parameterLines.set(
                'Reference:',
                `<a target="_blank" rel="noopener" href='${attrs['references']}'>${attrs['references']}</a>`
            );
        } else if (
            this.selectedParameter &&
            Object.keys(this.selectedParameter.sourceData.attrs).length > 0 &&
            updateParameterInfo
        ) {
            ...
            if (this.cubeTags.includes(CubeTag.CamsEac4Reanalysis)) {
                ...
                parameterLines.set(
                    'Dataset:',
                    'CAMS global reanalysis (EAC4) monthly averaged, ECMWF (' +
                    makeLink(
                ...
                parameterLines.set(
                    'Dataset:',
                    'ERA5 monthly averaged data on pressure levels from 1940 to present, ECMWF (' +
                    makeLink(
        ...
        if (!this.context.widgetMode) {
            cornerLineSmall(
                `<div>When using Lexcube and/or generated images or videos, please acknowledge/cite: M. Söchting et al., doi: <a href="https://doi.org/10.1109/MCG.2023.3321989" target="blank" onclick="return true">10.1109/MCG.2023.3321989</a>.</div>`
            );
        }
        ...
        if (this.cubeTags.includes(CubeTag.ESDC2)) {
            ...
            dialogParameterValue(
                'All data was postprocessed and merged in the Earth System Data Cube v2.1.1 as part of the ESA (Deep) Earth System Data Lab project.'
            );
        } else if (this.cubeTags.includes(CubeTag.ESDC3)) {
            dialogParameterHeading('Data Integration');
            dialogParameterValue(
                'All data was postprocessed and merged in the Earth System Data Cube v3.0.2 as part of the ESA (Deep) Earth System Data Lab project.'
            );
        } else if (this.cubeTags.includes(CubeTag.SpectralIndices)) {
            dialogParameterHeading('Data Source');
            if (this.selectedCube.shortName.indexOf('Sentinel-2') > -1) {
                dialogParameterValue(
                    'MSI, Sentinel-2 L2A, Copernicus, ESA. <a target=\'_blank\' rel=\'noopener noreferrer\' href=\'https://sentinels.copernicus.eu/web/sentinel/missions/sentinel-2\'>https://sentinels.copernicus.eu/web/sentinel/missions/sentinel-2</a>'
                );
            } else if (this.selectedCube.shortName.indexOf('MODIS') > -1) {
                dialogParameterValue(
                    'MODIS, Terra-Aqua, LP DAAC, USGS. <a target=\'_blank\' rel=\'noopener noreferrer\' href=\'https://lpdaac.usgs.gov/data/get-started-data/collection-overview/missions/modis-overview/\'>https://lpdaac.usgs.gov/data/get-started-data/collection-overview/missions/modis-overview/</a>'
                );
            } else if (this.selectedCube.shortName.indexOf('Planet Fusion') > -1) {
                dialogParameterValue(
                    'Planet-Fusion, Planet. <a target=\'_blank\' rel=\'noopener noreferrer\' href=\'https://www.planet.com/products/monitoring/\'>https://www.planet.com/products/monitoring/</a>'
                );
            }
            dialogParameterHeading('Data Integration');
            dialogParameterValue(
                'Spectral indices data calculated and aggregated by <a target=\'_blank\' href=\'https://rsc4earth.de/authors/dmontero/\'>David Montero</a> using his open-source <a target=\'_blank\' href=\'https://github.com/awesome-spectral-indices/awesome-spectral-indices\'>awesome-spectral-indices</a> libraries.'
            );
        }
        dialogParameterHeading('Data Cube Concept');
        dialogParameterValue(
            'In this visualization, data cubes are displayed as space-time cubes with the time axis extending into the background. For more information on the data cube concept, see <a target=\'_blank\' href=\'https://esd.copernicus.org/articles/11/201/2020/\'>Earth System Data Cubes Unravel Global Multivariate Dynamics by Mahecha et al. (2020)</a>.'
        );
        dialogParameterHeading('Data Visualization');
        dialogParameterValue(
            'A PhD project by Maximilian Söchting; advisors Miguel Mahecha & Gerik Scheuermann, Leipzig University, a collaboration between the Environmental Data Science and Remote Sensing group (Institute for Earth System Science and Remote Sensing) and the Image and Signal Processing Group (Institute for Computer Science). Lexcube is also available for Jupyter notebooks on <a target=\'_blank\' href=\'https://www.github.com/msoechting/lexcube\'>GitHub</a>. Region borders from Natural Earth.'
        );
        dialogParameterHeading('Funding');
        dialogParameterValue(
            'This project is supported by the National Research Data Infrastructure for Earth System Sciences NFDI4Earth (pilot projects), the German Science Foundation (DFG) and the European Space Agency (ESA) via the DeepExtremes and DeepESDL projects.'
        );
    ...
    selectParameter(parameterId: string, cubeChanged: boolean = false) {
        ...
        this.context.networking.cancelAllRequests();
        ...
        if (this.cubeTags.includes(CubeTag.ColormappingFromObservedValues)) {
            // if using observed values for color mapping
        ...
        if (this.context.scriptedMultiViewMode) {
            ...
            const pos = new Vector2(
                0.5 + (parameterIndex % columns),
                0.5 + Math.floor(parameterIndex / columns)
            );
            const spacing = 0.2;
            const cubeSize = 1;
            ...
            const position = new Vector3(
                0,
                -(-columnSize / 2 + pos.y * (cubeSize + spacing)),
                -(-rowSize / 2 + pos.x * (cubeSize + spacing))
            );
        ...
        if (!this.context.widgetMode) {
            this.htmlColormapMinInputDiv.value =
                this.selectedParameter.fixedColormapMinimumValue !== undefined
                    ? `${this.selectedParameter.fixedColormapMinimumValue}`
                    : !this.cubeTags.includes(CubeTag.ColormappingFromObservedValues)
                        ? `${this.selectedParameter.globalMinimumValue}`
                        : '';
            this.htmlColormapMaxInputDiv.value =
                this.selectedParameter.fixedColormapMaximumValue !== undefined
                    ? `${this.selectedParameter.fixedColormapMaximumValue}`
                    : !this.cubeTags.includes(CubeTag.ColormappingFromObservedValues)
                        ? `${this.selectedParameter.globalMaximumValue}`
                        : '';
        }
        ...
        if (this.selectedParameter.fixedColormap !== undefined) {
            const flipped =
                this.selectedParameter.fixedColormapFlipped !== undefined &&
                this.selectedParameter.fixedColormapFlipped;
            ...
            if (this.selectedParameter.isAnomalyParameter()) {
                ...
                if (this.context.widgetMode) {
                    ...
                    this.selectArbitraryLinearColormap(
                        this.htmlParameterSelect.options.selectedIndex + 2
                    );
                }
            }
        }
        ...
        this.animationParameters = new AnimationParameters(
            Dimension.Z,
            this.cubeDimensions,
            this.selectedCubeMetadata.sparsity,
            this.updateAnimationDurationLabel.bind(this)
        );
    ...
    async selectCube(logicalDataCube: LogicalDataCube) {
        ...
        for (let parameterId of Object.keys(meta['data_vars'])) {
            const parameterAttributionLookupId = parameterId.endsWith(ANOMALY_PARAMETER_ID_SUFFIX)
                ? parameterId.substring(0, parameterId.length - ANOMALY_PARAMETER_ID_SUFFIX.length)
                : parameterId;
            const attribution = this.getAttributionParameterMetadata(parameterAttributionLookupId);
            const parameter = new Parameter(
                parameterId,
                this.selectedCubeMetadata.data_vars[parameterId],
                attribution,
                this.getColormapParameterMetadata(parameterId)
            );
        ...
        if (
            this.cubeDimensions.x.type == CubeDimensionType.Longitude &&
            this.cubeDimensions.y.type == CubeDimensionType.Latitude &&
            this.cubeDimensions.x.getValueRange() > 350 &&
            this.cubeDimensions.y.getValueRange() > 170
        ) {
            this.context.log(
                'X/Y are longitude/latitude with value ranges > 350 / 170, assuming cube is global.'
            );
            this.cubeTags.push(CubeTag.Global);
            this.cubeTags.push(CubeTag.OverflowX);
            if (this.cubeDimensions.x.getMaxValue() > 350) {
                this.cubeTags.push(CubeTag.LongitudeZeroIndexIsGreenwich);
                this.context.log(
                    'Found longitude values > 350, will assume longitude zero index is Greenwich'
                );
            }
        }
        ...
        if (this.cubeTags.includes(CubeTag.LongitudeZeroIndexIsGreenwich)) {
            // works so far for CAMS EAC4 and ERA5
            xCorrection = GeospatialContextCorrection.AddFullStepAtEnd;
            yCorrection = GeospatialContextCorrection.None;
        }

        const message = geospatialContext.setFromDimensions(
            this.cubeDimensions,
            xCorrection,
            yCorrection
        );
        ...
        if (this.cubeTags.includes(CubeTag.Global) && !geospatialContext.isValid()) {
            if (this.cubeDimensions.y.type != CubeDimensionType.Latitude) {
                console.warn(
                    'Y dimension is not Latitude, but trying to use it for guessing global coverage'
                );
            }
            geospatialContext.setGlobalCoverage();
            this.context.log(
                'Guessing geospatial context, assuming equally distributed global coverage'
            );
        }
        this.context.log(
            'Geospatial context provided:',
            geospatialContext,
            'isValid:',
            geospatialContext.isValid()
        );
        ...
        this.context.rendering.updateOverflowSettings(
            this.cubeTags.includes(CubeTag.OverflowX),
            false,
            false
        );
        this.XYdataAspectRatio = this.context.widgetMode
            ? 1.0
            : this.cubeDimensions.x.steps / this.cubeDimensions.y.steps;
        this.context.log('XYdataAspectRatio:', this.XYdataAspectRatio);
        this.context.log(
            'Cube tags:',
            this.cubeTags.map((a) => CubeTag[a])
        );
        ...
        if (
            this.initialLoad &&
            this.initialSelectionState.parameterId &&
            this.parseInitialParameter()
        ) {
            ...
            this.selectParameter(
                this.cubeParameters.get(Array.from(this.cubeParameters.keys())[0])!.name,
                true
            );
        }
        if (!this.context.widgetMode) {
    ...
    private parseInitialParameter() {
        const s = Array.from(this.cubeParameters.keys()).find(
            (s) => s.toLowerCase() == this.initialSelectionState.parameterId
        );
        if (s) {
    ...
    applyCameraPreset(
        presetName: string = '',
        cameraOverride: PerspectiveCamera | OrthographicCamera | undefined = undefined,
        presetOverride:
            | {
        ...
        if (presetName.length > 0) {
            ...
            presetIndex = this.cameraPresets.findIndex(
                (c) => c.name == `Single Face (${urlPreset[1]})`
            );
        }
        const c = presetOverride || this.cameraPresets[presetIndex];
        this.context.log(
            'Applying camera preset',
            'name' in c ? c.name : `custom preset (${JSON.stringify(c)})`
        );
        ...
        if (
            ...
            !this.context.rendering.printTemplateDownloading &&
            !this.context.isometricMode &&
            !presetOverride
        ) {
    ...
    private updateAvailableParametersUi() {
        ...
        if (this.cubeTags.includes(CubeTag.CamsEac4Reanalysis)) {
            groupMapper = (p: Parameter) => {
                if (p.longName.toLowerCase().includes('vertically integrated'))
                    return 'Vertically Integrated Masses';
                if (p.longName.toLowerCase().includes('total column')) return 'Atmospheric Columns';
                if (
                    p.longName.toLowerCase().includes('aerosol') ||
                    p.longName.toLowerCase().includes('particulate')
                )
                    return 'Aerosols';
                return 'Meteorology';
            };
        ...
        for (let key of sortedKeys) {
            ...
            for (let parameterId of group) {
                ...
                if (this.cubeTags.includes(CubeTag.SpectralIndices)) {
                    ...
                    option.text =
                        attributionMetadata && attributionMetadata.long_name
                            ? attributionMetadata.long_name
                            : parameter!.longName || parameterId;
                }
                option.text += suffix;
    ...
    updateDisplaySignificance() {
        ...
        const n =
            3 -
            Math.log10(
                this.selectedParameter.getConvertedDataValue(td.observedMaxValue) -
                this.selectedParameter.getConvertedDataValue(td.observedMinValue)
            );
        if (isNaN(n)) {
            return;
        }
    ...
    getColormapDataFromName(name: string) {
        const category = Object.keys(defaultColormaps).find((c) =>
            Object.keys((defaultColormaps as any)[c]).includes(name)
        )!;
    ...
    private updateColormapScale(data: number[][]) {
        const gradient = this.colormapScaleCanvasContext.createLinearGradient(
            0,
            0,
            this.colormapScaleWidth,
            0
        );
        for (let i = 0; i < data.length; i++) {
        ...
        this.colormapScaleCanvasContext.fillRect(
            0,
            0,
            this.colormapScaleWidth,
            this.colormapScaleHeight
        );
    }
    ...
    updateColormapScaleTexts(minValue: number, maxValue: number) {
        ...
        for (let i = 0; i < count; i++) {
            const p = i / (count - 1);
            this.htmlColormapScaleTexts[i].textContent =
                `${this.toFixed(this.selectedParameter.getConvertedDataValue(p * (maxValue - minValue) + minValue))}`;
        }
    }
        ...
        for (let category of colormapCategories.keys()) {
            ...
            colormapCategoryHeader.onclick = () => {
                if (colormapCategoryHeader.getAttribute('collapsed') == 'true') {
                    ...
                    colormapCategoryHeader.scrollIntoView({
                        block: this.context.widgetMode ? 'nearest' : 'center'
                    });
                } else {
                    colormapCategoryHeader.textContent = `► ${categoryName}`;
            ...
            for (let j = 0; j < colormapNames.length; j++) {
                ...
                button.onclick = () => {
                    this.selectColormapByName(name);
                };
                button.title = name;
    ...
    private getDisplaySizeBounds(face: CubeFace) {
        ...
        const dataAspectRatio =
            ...
            face == CubeFace.Top ||
            face == CubeFace.Bottom
                ? this.XYdataAspectRatio
                : 1.0;
        const minDisplaySize = new Vector2(
            (width * Math.pow(0.5, MAX_ZOOM_FACTOR)) / dataAspectRatio,
            height * Math.pow(0.5, MAX_ZOOM_FACTOR)
        );
        let maxDisplaySize = new Vector2();
        if (dataAspectRatio > 1.0) {
            maxDisplaySize = new Vector2(
                this.roundDownToSparsity(width / dataAspectRatio),
                this.roundDownToSparsity(height)
            );
        } else {
            maxDisplaySize = new Vector2(
                this.roundDownToSparsity(width),
                this.roundDownToSparsity(height * dataAspectRatio)
            );
        }
        return { maxDisplaySize, minDisplaySize };
    }

    private changeZoomOnFace(
        zoomDelta: number,
        face: CubeFace,
        focusUv: Vector2,
        ...
        this.currentZoomFactor[Math.floor(face / 2)] = clamp(
            this.currentZoomFactor[Math.floor(face / 2)] + zoomDelta / 200,
            1.0,
            MAX_ZOOM_FACTOR
        );
        const zoomFactor = Math.pow(0.5, this.currentZoomFactor[Math.floor(face / 2)] - 1.0);
        const zoomFactorDifference = Math.abs(
            this.currentZoomFactor[Math.floor(face / 2)] - previousZoomFactor
        );
        const zoomFactorChanged = Math.abs(zoomFactorDifference) > 0.01;
        let newCenterPoint = new Vector2();
        const oldCenterPoint = oldDisplayOffset
            .clone()
            .add(oldDisplaySize.clone().multiplyScalar(0.5));
        if (immediate) {
            let normalizeFactor = 1.0;
            if (Math.sign(zoomDelta) == 1) {
                // zoom in
                normalizeFactor = 1.0;
            } else {
                // zoom out
            ...
            newCenterPoint = oldDisplayOffset.clone().add(
                oldDisplaySize.clone().multiply(
                    focusUv
                        .clone()
                        .addScalar(0.5 * normalizeFactor)
                        .divideScalar(normalizeFactor + 1)
                )
            );
        } else {
            if (!this.currentZoomNewCenterPoint || !this.currentZoomOldCenterPoint) {
                this.currentZoomOldCenterPoint = oldDisplayOffset
                    .clone()
                    .add(oldDisplaySize.clone().multiplyScalar(0.5));
                this.currentZoomNewCenterPoint = oldDisplayOffset
                    .clone()
                    .add(oldDisplaySize.clone().multiply(focusUv));
            }
            const zoomFactorDistance = clamp(zoomFactorDifference, 0.0, 2.0);
            const p = zoomFactorDistance / 2.0;
            newCenterPoint = this.currentZoomOldCenterPoint
                .clone()
                .multiplyScalar(1 - p)
                .add(this.currentZoomNewCenterPoint.clone().multiplyScalar(p));
        }
        ...
        this.triggerMaxRangeIndicatorsFromSizeIfNecessary(
            face,
            newDisplaySize,
            displaySizeBounds.maxDisplaySize
        );
        ...
        this.triggerMaxRangeIndicatorsFromOffsetIfTouchEdges(
            face,
            this.cubeSelection.getOffsetVector(face),
            this.cubeSelection.getSizeVector(face)
        );
    }

    private triggerMaxRangeIndicatorsFromSizeIfNecessary(
        face: CubeFace,
        unclampedNewDisplaySize: Vector2,
        maxDisplaySize: Vector2
    ) {
        const xOverflowEnabled = this.getXOverflowEnabledForFace(face);
    ...
    private triggerMaxRangeIndicatorsFromOffsetIfScrolledOutSignificantly(
        face: CubeFace,
        unclampedNewDisplayOffset: Vector2,
        displaySize: Vector2
    ) {
        const minimumDisplayOffset = this.getMinimumDisplayOffset(face);
        const maximumDisplayOffset = this.getMaximumDisplayOffset(face, displaySize);
        const xOverflowEnabled = this.getXOverflowEnabledForFace(face);
        const xScrolledOutside = xOverflowEnabled
            ? 0
            : Math.max(
                minimumDisplayOffset.x - unclampedNewDisplayOffset.x,
                unclampedNewDisplayOffset.x - maximumDisplayOffset.x,
                0
            );
        const yScrolledOutSide = Math.max(
            minimumDisplayOffset.y - unclampedNewDisplayOffset.y,
            unclampedNewDisplayOffset.y - maximumDisplayOffset.y,
            0
        );
        ...
        if (
            xScrolledOutsideSignificantly &&
            unclampedNewDisplayOffset.x < minimumDisplayOffset.x &&
            !xOverflowEnabled
        ) {
            this.context.rendering.showMaxRangeIndicator(face, Dimension.X, true);
        }
    ...
    private triggerMaxRangeIndicatorsFromOffsetIfTouchEdges(
        face: CubeFace,
        clampedNewDisplayOffset: Vector2,
        displaySize: Vector2
    ) {
        const minimumDisplayOffset = this.getMinimumDisplayOffset(face);
        const maximumDisplayOffset = this.getMaximumDisplayOffset(face, displaySize);
    ...
    reconstructZoomFactor(face: CubeFace, onlyAssignIfDifferent: boolean = false) {
        ...
        const newZoomFactor = clamp(
            1.0 - Math.log2(Math.min(displaySize.x, displaySize.y)),
            1.0,
            MAX_ZOOM_FACTOR
        );
        const zoomFactorDifference = Math.abs(
            newZoomFactor - this.currentZoomFactor[Math.floor(face / 2)]
        );
    ...
    getVisibleFaces() {
        ...
        for (let face = 0; face < 6; face++) {
            ...
            if (!visible) {
                // maybe instead prioritize by visibility?
                continue;
            }
            result.push(face);
        }
    ...
    getVisibleTiles() {
        ...
        for (let face = 0; face < 6; face++) {
            ...
            const maxVisibleX = Math.floor(
                positiveModulo(offset.x + size.x - 1, width) / lodTileSize
            );
            const xOverflown =
                Math.floor(offset.x / width) < Math.floor((offset.x + size.x) / width);
            if (
                (face == CubeFace.Front ||
                    face == CubeFace.Back ||
                    face == CubeFace.Top ||
    ...
    async triggerTileDownloads() {
        ...
        this.context.networking.updateViewBlocks(visibleTiles);
        const tilesToDownload = visibleTiles.filter(
            (t) => !this.context.tileData.isTileDownloadTriggered(t)
        );
        ...
        const finishedTiles = visibleTiles.filter((t) =>
            this.context.tileData.isTileDownloadFinished(t)
        );
        const faces = this.getVisibleFaces();
        for (let face of faces) {
            if (
                finishedTiles.filter((t) => t.face == face).length ==
                visibleTiles.filter((t) => t.face == face).length
            ) {
    ...
    private nextAnimationStep() {
        ...
        this.updateSlidersAndLabelsAfterChange(true, true, [
            this.animationParameters.getDimension()
        ]);
        this.context.rendering.updateVisibilityAndLods();
    ...
    getColormapMinMaxValuePrecision() {
        ...
        if (
            this.cubeTags.includes(CubeTag.Era5SpecificHumidity) &&
            !this.selectedParameter.isAnomalyParameter()
        ) {
            return 5;
        }
        return Infinity;
    ...
    updateRequestProgressFromWidget(
        progress: number[],
        isReliableProgressForTiming: boolean = false
    ) {
    ...
    async showPrintTemplateResult(svg: string) {
        ...
        if (this.context.widgetMode) {
            ...
            const qr = await QRCode.toDataURL(document.URL, {
                color: { dark: '#000', light: '#ffffff00' }
            });
        ...
        svg = svg.replace(
            '%parameter%',
            this.selectedParameter.longName || this.selectedParameter.name
        );
        ...
        this.htmlPrintTemplateDownloadEditNoteButton.onclick = () => {
            const input =
                window.prompt(
                    this.htmlPrintTemplateFirstNote
                        ? 'Add a custom note:'
                        : 'Edit your custom note:'
                ) || '';
            if (input == '') {
                return;
            }
    ...
    async showNewPrintTemplateResult(svg: string) {
        ...
        reader.onload = (e) => {
            ...
            svgImage.onload = () => {
                ...
                this.context.log(
                    'Print template all done (SVG + PNG), took',
    ...
    private setAnimationUseSelectedRangeOnly() {
        this.animationParameters.setUseSelectedRange(
            this.htmlAnimationSelectedRangeOnlyCheckbox.checked
        );
        this.updateAnimationSliders();
    }
