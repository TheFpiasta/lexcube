...
import { NumcodecsWorker } from 'numcodecs';
import { TILE_FORMAT_MAGIC_BYTES, TILE_VERSION, TILE_VERSION_3D, TILE_VERSION_MASK, TILE_VERSION_FLAG_FLOAT32, TILE_VERSION_FLAG_FLOAT64, TILE_VERSION_FLAG_RGB_UINT8, NAN_TILE_MAGIC_NUMBER, LOSSLESS_TILE_MAGIC_NUMBER, TILE_SIZE_2D, TILE_SIZE_3D } from '../constants';
...
export class TileDecompressor {
    private readonly WORKER_COUNT = 1;
    private workers: NumcodecsWorker[] = new Array(this.WORKER_COUNT).fill(undefined).map(() => new NumcodecsWorker({ poolSize: 1 }));
     ...
     */
    async decompressTile(tile: Tile2D | Tile3D, data: ArrayBuffer, debugMode: boolean = false): Promise<DecompressedTileResult> {
        ...
        if ((is2d && tile_version !== TILE_VERSION) || (is3d && tile_version !== TILE_VERSION_3D)) {
            console.error("Received tile has invalid tile version:", tile_version, "Expected:", is2d ? TILE_VERSION : TILE_VERSION_3D);
            return { validTile: false };
        }

        let expectedDtype: typeof Float32Array | typeof Float64Array | typeof Uint8Array | null =
            (tile_version_and_flags & TILE_VERSION_FLAG_FLOAT32) ? Float32Array :
            (tile_version_and_flags & TILE_VERSION_FLAG_FLOAT64) ? Float64Array :
            (tile_version_and_flags & TILE_VERSION_FLAG_RGB_UINT8) ? Uint8Array : null;

        if (tile_version_and_flags == tile_version) {
            if (debugMode) {
                console.warn("Legacy tiles without data type flag received, assuming Float64-ZFP-encoded and Float32 when decoded");
            }
            expectedDtype = Float32Array;
        } else if (!expectedDtype) {
            console.error("Received tile has invalid or unsupported data type flag in tile version and flags:", tile_version_and_flags);
            return { validTile: false };
        }
        ...
        const nanOrQuantileMaskHeaderBytes = new Uint8Array(data, 56, nanOrQuantileMaskHeaderLength);
        ...
        this.nextWorker = (this.nextWorker + 1) % this.WORKER_COUNT;

        const nanMaskDecode = worker.lz4Decode;
        const losslessTileDecode = worker.bloscDecode;
        const defaultTileDecode = is3d ? worker.zfpDecode3d : worker.zfpDecode;
        ...
        if (lossless) {
            if (is3d) {
                const result = await Promise.all([
                    nanMaskDecode(nanOrQuantileMaskHeaderBytes),
                    losslessTileDecode(tileDataBytes)
                ]);
                nanOrQuantileMask = result[0] as Uint8Array;
                tileData = result[1] as Uint8Array;
            } else {
                tileData = await losslessTileDecode(tileDataBytes) as Uint8Array;
            }
        } else {
            const result = await Promise.all([
                nanMaskDecode(nanOrQuantileMaskHeaderBytes),
                defaultTileDecode(tileDataBytes)
            ]);
            nanOrQuantileMask = result[0] as Uint8Array;
            tileData = result[1] as Uint8Array;

            releaseBuffer = () => {
                worker.release(tileData.buffer); // release ZFP output buffer back to worker pool
            }
        }
